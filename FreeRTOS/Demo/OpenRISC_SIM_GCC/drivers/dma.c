...
int dma_channel_set(int channel,
                    unsigned int src_interface,
                    unsigned int src_addr,
                    unsigned int src_addr_incr,
                    unsigned int dst_interface,
                    unsigned int dst_addr,
                    unsigned int dst_addr_incr,
                    unsigned int chunk_word,
                    unsigned int transfer_word,
                    int handshake_mode,
                    int int_enable)
{
    ...
    channel_transfer_size =
        (transfer_word << DMA_CHAN_TOT_SZ_SHT) & DMA_CHAN_TOT_SZ_MSK |
    ...
    do {
    // do nothing
    } while(!(_dma->channels[channel].control &
          (DMA_CHAN_DONE_MSK | DMA_CHAN_ERR_MSK)));
